...
def query_rewrite_node(state: RetrievalState) -> dict:
    ...
    if company_status:
        ...
        for c, found in company_status.items():
            if not found:
                missing.append(c)
        ...
        base_query = state.get("rewritten_query") or query
        unsupported_hints = state.get("unsupported_hints") or []
        company_queries = []

        for c in missing:
            company_hints = [h for h in unsupported_hints if c in h]
            if company_hints:
                query_str = " ".join(company_hints)
            else:
                query_str = f"{c} {base_query}"
