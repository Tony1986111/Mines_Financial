...
export default function ProgressCard({ card }: { card: ProgressCardData }) {
  ...
  const base =
    `min-w-0 rounded-xl px-3.5 py-2.5 text-xs leading-relaxed flex items-start gap-2.5 border transition-all duration-300 ${
  ...
  if (card.status === "running") {
    return (
      <div className={`${base} relative overflow-hidden bg-[#1a4a8a]/[0.07] dark:bg-[#0d1c2e] border-[#b0c8e0]/40 dark:border-[#162840] text-[#1a4a8a] dark:text-[#7aade8]`}>
        ...
        <span className="relative min-w-0 break-words">{card.text}</span>
      </div>
    );
  }
  ...
  if (card.variant === "direct") {
    return (
      <div className={`${base} bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-800/30 text-emerald-700 dark:text-emerald-400`}>
        ...
        <span className="min-w-0 break-words">{card.text}</span>
      </div>
    );
  }
  ...
  if (card.variant === "tools") {
    return (
      <div className={`${base} flex-wrap bg-[#f4f8fd] dark:bg-[#0d1c2e] border-[#cddcea] dark:border-[#162840] text-[#0a1e38] dark:text-[#c4d8f0]`}>
        ...
        <div className="min-w-0 flex flex-wrap gap-x-1 items-center">
  ...
  return (
    <div className={`${base} flex-col gap-1.5 bg-white dark:bg-[#0d1c2e] border-[#cddcea] dark:border-[#162840] shadow-sm`}>
      <div
        className={`flex min-w-0 items-center gap-2 font-semibold text-[#1a4a8a] dark:text-[#7aade8] ${isExpandable ? "cursor-pointer select-none" : ""}`}
      ...
      {card.bodyLines && card.bodyLines.length > 0 && (
        <div className="flex min-w-0 flex-wrap gap-1.5 pl-0 md:pl-6">
          {card.bodyLines.map((line, i) => (
            <span
              key={i}
              className="inline-block max-w-full break-words bg-[#fef6e4] dark:bg-[#1a1004] border border-[#e8c86a] dark:border-[#3d2800] rounded px-2 py-0.5 text-[11px] font-mono font-semibold text-[#966000] dark:text-[#c4880c]"
            >
              {line}
            </span>
          ))}
      ...
      {isExpandable && expanded && (
        <div className="pl-0 md:pl-6 text-[11px] text-[#7a9ab8] dark:text-[#5a7898] leading-relaxed border-t border-[#e0eaf5] dark:border-[#1e3858] pt-1.5 mt-0.5 whitespace-pre-wrap break-words">
          {card.expandBody}
        </div>
      )}
    </div>
